...
export function createApp(db: Database.Database, dataDir: string): express.Express {
  ...
  api.post('/templates', (req, res) => {
    ...
    const r = db
      .prepare(
        'INSERT INTO templates (nom, largeur_mm, hauteur_mm, dlc_jours, quantite_carton, poids_g, position) VALUES (?, 85, 55, 365, 15, 280, (SELECT COALESCE(MIN(position), 1) - 1 FROM templates))'
      )
      .run(nom)
  ...
  api.put('/templates/:id', (req, res) => {
    const t = templateParId.get(req.params.id)
    if (!t) return res.status(404).json({ erreur: 'template introuvable' })
    const champs = ['nom', 'largeur_mm', 'hauteur_mm', 'dlc_jours', 'quantite_carton', 'poids_g', 'doc_json', 'vignette_png', 'categorie', 'position']
  ...
  api.post('/templates/:id/duplicate', (req, res) => {
    ...
    const r = db
      .prepare(
        'INSERT INTO templates (nom, largeur_mm, hauteur_mm, dlc_jours, quantite_carton, poids_g, doc_json, vignette_png, categorie, position) VALUES (?,?,?,?,?,?,?,?,?,?)'
      )
      .run(nom, t.largeur_mm, t.hauteur_mm, t.dlc_jours, t.quantite_carton, t.poids_g, t.doc_json, t.vignette_png, t.categorie, t.position)
    res.status(201).json(templateParId.get(r.lastInsertRowid))
  })
